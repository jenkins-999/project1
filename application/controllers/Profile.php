...
class Profile extends CI_Controller {
    ...
    function __construct(){
        ...
        $user = $this->session->userdata('user');
            if(empty($user)) {
                $this->session->set_flashdata('msg', 'Sesi telah kadaluarsa silahkan login kembali.');
        ...
        if($this->form_validation->run() == true) { 
            ...
            if(password_verify($cPassword, $user['password']) == true) {
                if($nPassword != $nRPassword) {
                    $this->session->set_flashdata('pwd_error', 'password harus sama');
                    redirect(base_url(). 'profile/index');
                }else {
                ...
                $this->session->set_flashdata('pwd_error', 'Password lama anda salah');
                redirect(base_url(). 'profile/index');
            }
        }else {
            $data['user'] = $user; 
